...
resource "azurerm_windows_virtual_machine" "vm_wus3" {
  #checkov:skip=CKV_AZURE_50:Example code - VM extensions not in scope
  #checkov:skip=CKV_AZURE_151:Example code - VM disk encryption not in scope
...
resource "azurerm_managed_disk" "vm_wus3" {
  #checkov:skip=CKV_AZURE_93:Example code - disk encryption set not in scope
  create_option        = "Empty"
  location             = azurerm_resource_group.primary_wus3.location
