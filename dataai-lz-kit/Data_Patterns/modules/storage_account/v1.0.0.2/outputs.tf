...
output "data_lake_gen2_paths" {
  description = "Map of Data Lake Gen2 paths (nested directories/files) that are created."
  value = {
    for key, p in azurerm_storage_data_lake_gen2_path.this :
    key => {
      id   = p.id
      path = p.path
    }
