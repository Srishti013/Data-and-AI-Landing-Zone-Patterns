...
variable "deployment_slots" {
  type = map(object({
    ...
    # https_only                               = optional(bool, false) //disabling the https_only to allow slot swapping with https_only enabled slots
    key_vault_reference_identity_id = optional(string, null)
    # managed_identities = optional(object({
    #   system_assigned            = optional(bool, false)
    #   user_assigned_resource_ids = optional(set(string), [])
    # }), {})
    # public_network_access_enabled                  = optional(bool, true) //disabling the public network access to allow slot swapping with public network access disabled slots
