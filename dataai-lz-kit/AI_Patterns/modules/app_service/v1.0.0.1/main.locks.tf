resource "azurerm_management_lock" "this" {
  ...
  name       = coalesce(var.lock.name, "lock-${module.module_appservice.name}")
...
resource "azurerm_management_lock" "pe" {
  ...
  name       = each.value.lock != null ? each.value.lock.name : (each.value.name != null ? "lock-${each.value.name}" : "lock-pe-${module.module_appservice.name}")
...
resource "azurerm_management_lock" "slot" {
  ...
  name       = "lock-${coalesce(each.value.name, "slot-${module.module_appservice.name}")}"
