...
- Validate workload-specific settings before moving from `v1.0.0.0` to `v1.0.0.1`, especially networking, authentication, and private endpoint behaviour.
...
- Public access, private endpoint access, and slot-specific private endpoints are supported depending on configuration, but `v1.0.0.1` is intended for private-access-first deployments.
...
- HTTPS-only traffic and hardened App Service security defaults are enforced in this version.
...
- This module creates one App Service workload and can optionally configure deployment slots, private endpoints, Application Insights, diagnostics, custom domains, authentication, and resource locks.
...
| Name | Source | Version |
|------|--------|---------|
| mbb_app_service | [IAC link](https://github.com/maybank-ghes/mbb-az-iac-modules/tree/main/modules/mbb_app_service) | v1.0.0.1 |
...
```main.tf
module "app_service" {
  source = "../../modules/mbb_app_service/v1.0.0.1"
