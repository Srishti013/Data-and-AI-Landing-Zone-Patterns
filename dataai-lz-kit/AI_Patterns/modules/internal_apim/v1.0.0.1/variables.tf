variable "global_policy_vars" {
  type = object({
    jwt_header_name        = string
    jwt_failed_status_code = number
    jwt_failed_message     = string
    jwt_require_expiry     = bool
    jwt_scheme             = string
  ...
  validation {
    condition     = var.global_policy_vars.jwt_failed_status_code == 401
    error_message = "JWT failed status code must be 401."
  }

  validation {
    condition     = var.global_policy_vars.jwt_scheme == "Bearer"
    error_message = "JWT scheme must be 'Bearer'."
  }

  validation {
    condition     = var.global_policy_vars.rate_limit_calls > 0
    error_message = "Rate limit calls must be greater than 0."
  }

  validation {
    condition     = var.global_policy_vars.quota_calls > 0
    error_message = "Quota calls must be greater than 0."
  }

  # validation {
  #   condition     = contains(["information", "warning", "error"], var.global_policy_vars.log_severity)
  #   error_message = "Log severity must be information, warning, or error."
  # }
}
...
- `protocols` - protocols are enforced by the module security baseline (HTTPS only).
...
variable "backends" {
  description = "Map of APIM backend configurations"
  type = map(object({
...
DESCRIPTION
  ...
  # }
  validation {
    condition = length(var.private_endpoints) == 0 || can(regex("StandardV2", var.sku_name))

    error_message = "Private endpoints are only supported with StandardV2 SKU."
  }
}
...
# variable "security" { // MBB - removed to disable Variable - Security settings for the API Management service.
#   type = object({
...
# variable "virtual_network_type" { //hardcoded in the code to internal for internal apim
#   type        = string
#   default     = "None"
