...
<!-- BEGIN_TF_DOCS -->
# Azure Naming Module v1.0.0.1 (Simplified)

## Overview

This {org} module provides the following features:

- From a {org} `region_code`, the module gives the Azure `location name` to use in resource's creation,
- From the {org} inputs (org, country, env, base\_name, etc.) and an Azure resource abbreviation ([Recommended abbreviations for Azure resource types](https://docs.microsoft.com/en-us/azure/cloud-adoption-framework/ready/azure-best-practices/resource-abbreviations)), the module generates the resource name, according to this {org} Naming Standard:
...
From the inputs provided to the {org} module, the following `tags` are automatically generated:
...
The management of the `created_on` tag is done with the help of the `time` Terraform provider. It captures the **FIRST time** the {org} module was used to generate the name and stores the date in both the tag value and the module state.

## Example

This module is used within another module to generate the resource(s) name(s) through {org} naming convention, when relevant for the resource(s).
To use it:
...
### 1. Call the {org} module

```yaml
module "module_rg" {
  # Local use
  #source = "../../terraform-azurerm-{org}-module"

  # Terraform Cloud/Enterprise use
  source  = "app.terraform.io/msft{org}deo/{org}-module/azurerm"
  version = "1.0.1"
...
|------|-------------|------|---------|:--------:|
| <a name="input_au"></a> [au](#input\_au) | (Required) {org} Accounting Unit (AU) code. Example: `0233985`. <br></br>&#8226; Value of `au` must be of numeric characters. | `string` | n/a | yes |
...
| <a name="input_env"></a> [env](#input\_env) | (Required) {org} environment code. Example: `test`. <br></br>&#8226; Value of `env` must be one of: `[nonprod,prod,core,int,uat,stage,dev,test]`. | `string` | n/a | yes |
| <a name="input_owner"></a> [owner](#input\_owner) | (Required) {org} technology owner group. | `string` | n/a | yes |
| <a name="input_resource_type_code"></a> [resource\_type\_code](#input\_resource\_type\_code) | (Required) Azure resource type abbreviation (or `service` in {org} Naming Standard). Example: `rg`, `vnet`, `st`, etc. More information: [Azure resource abbreviations](https://docs.microsoft.com/en-us/azure/cloud-adoption-framework/ready/azure-best-practices/resource-abbreviations) | `string` | n/a | yes |
...
| <a name="input_country"></a> [country](#input\_country) | (Optional) {org} country code. Example: `bk`. | `string` | `"bk"` | no |
...
| <a name="input_org"></a> [org](#input\_org) | (Optional) {org} organization code. Example: `{org}`. | `string` | `"{org}"` | no |
| <a name="input_region_code"></a> [region\_code](#input\_region\_code) | (Optional) {org} region code.<br></br>&#8226; Value of `region_code` must be one of: `[sea,ea,eu,myw,sg]`. | `string` | `"sa"` | no |
