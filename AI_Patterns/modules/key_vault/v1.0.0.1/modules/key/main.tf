resource "azurerm_key_vault_key" "this" {
  ...
  # When a rotation_policy is set, Azure Key Vault manages the expiration_date of each new key
  # version automatically (based on expire_after). Ignore changes to expiration_date so Terraform
  # does not perpetually revert it to a statically configured value after the key auto-rotates.
  lifecycle {
