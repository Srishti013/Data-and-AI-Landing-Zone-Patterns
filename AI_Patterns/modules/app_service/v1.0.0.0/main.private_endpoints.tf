...
resource "azurerm_private_endpoint" "this" {
  ...
  location                      = coalesce(each.value.location, module.module_appservice.location)
  name                          = each.value.name != null ? each.value.name : "pep-${module.module_appservice.name}"
  resource_group_name           = each.value.resource_group_name != null ? each.value.resource_group_name : var.resource_group_name
  subnet_id                     = each.value.subnet_resource_id
  custom_network_interface_name = each.value.network_interface_name
  tags                          = var.all_child_resources_inherit_tags ? merge(module.module_appservice.tags, var.tags, each.value.tags) : each.value.tags

  private_service_connection {
    is_manual_connection           = false
    name                           = each.value.private_service_connection_name != null ? each.value.private_service_connection_name : "pse-${module.module_appservice.name}"
...
resource "azurerm_private_endpoint" "slot" {
  ...
  location                      = coalesce(each.value.pe_value.location, module.module_appservice.location)
  name                          = each.value.pe_value.name != null ? each.value.pe_value.name : "pep-${module.module_appservice.name}"
  ...
  tags                          = var.all_child_resources_inherit_tags ? merge(module.module_appservice.tags, var.tags, each.value.pe_value.tags) : each.value.pe_value.tags

  private_service_connection {
    is_manual_connection           = false
    name                           = each.value.pe_value.private_service_connection_name != null ? each.value.pe_value.private_service_connection_name : "pse-${module.module_appservice.name}"
    ...
    subresource_names              = (var.kind == "functionapp" || var.kind == "webapp") ? (var.kind == "functionapp" && var.function_app_uses_fc1 == false ? (var.os_type == "Windows" ? ["sites-${coalesce(azurerm_windows_function_app_slot.this[each.value.slot_key].name, "pep-${module.module_appservice.name}")}"] : ["sites-${coalesce(azurerm_linux_function_app_slot.this[each.value.slot_key].name, "pep-${module.module_appservice.name}")}"]) : (var.os_type == "Windows" ? ["sites-${coalesce(azurerm_windows_web_app_slot.this[each.value.slot_key].name, "pep-${module.module_appservice.name}")}"] : ["sites-${coalesce(azurerm_linux_web_app_slot.this[each.value.slot_key].name, "pep-${module.module_appservice.name}")}"])) : null
  }
  dynamic "ip_configuration" {
