...
resource "azurerm_service_plan" "example" {
  ...
  sku_name            = "P1v2"
  tags = {
    app = local.naming.app_service.name_unique
  }
}
...
resource "azurerm_log_analytics_workspace" "example_development" {
  location            = azurerm_resource_group.example.location
  name                = "${local.naming.log_analytics_workspace.name}-development-env"
  resource_group_name = azurerm_resource_group.example.name
  retention_in_days   = 30
  sku                 = "PerGB2018"
}
...
module "avm_res_web_site" {
  ...
  # checkov:skip=CKV_AZURE_221: Not in scope for this example - function app public network access is controlled by example/module settings
  ...
  # checkov:skip=CKV_AZURE_14: Not in scope for this example - HTTPS redirect behavior is controlled by example/module settings
  ...
  # checkov:skip=CKV_AZURE_222: Not in scope for this example - web app public network access is controlled by example/module settings
  ...
  # checkov:skip=CKV_AZURE_153: Not in scope for this example - app service slot HTTPS redirect behavior is controlled by example/module settings
  source = "../../"
  ...
  slot_application_insights = {
    development = {
      name                  = "${local.naming.application_insights.name_unique}-development-env"
