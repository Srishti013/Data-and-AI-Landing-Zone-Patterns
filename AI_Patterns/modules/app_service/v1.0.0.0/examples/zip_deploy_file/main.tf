...
resource "azurerm_log_analytics_workspace" "example" {
  ...
  name                = "${local.naming.log_analytics_workspace.name}-zip"
...
module "avm_res_web_site" {
  ...
  # checkov:skip=CKV_AZURE_221: Not in scope for this example - public network access behavior is controlled by example configuration
  ...
  # checkov:skip=CKV_AZURE_80: Not in scope for this example - .NET version check not applicable to this function app example
  # checkov:skip=CKV_AZURE_66: Not in scope for this example - failed request tracing setting is controlled by example/module settings
