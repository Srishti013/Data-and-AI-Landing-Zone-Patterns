resource "azurerm_windows_function_app" "this" {
  ...
  location                                 = module.module_appservice.location
  name                                     = module.module_appservice.name
  ...
  tags                                           = merge(module.module_appservice.tags, var.tags != null ? var.tags : {})
  ...
  dynamic "backup" {
    ...
    content {
      name                = coalesce(backup.value.name, "${module.module_appservice.name}-backup")
...
resource "azurerm_function_app_flex_consumption" "this" {
  ...
  # key_vault_reference_identity_id is not supported by azurerm_function_app_flex_consumption.
  # It is set via azapi_update_resource.flex_functionapp_kv_reference_identity at the end of this file.
  # public_network_access_enabled                  = each.value.public_network_access_enabled
  public_network_access_enabled                  = false
...
resource "azapi_update_resource" "flex_functionapp_kv_reference_identity" {
  count = var.kind == "functionapp" && var.os_type == "Linux" && var.function_app_uses_fc1 == true ? 1 : 0
