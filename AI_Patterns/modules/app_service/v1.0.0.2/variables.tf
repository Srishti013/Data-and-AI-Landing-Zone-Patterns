...
variable "managed_identities" {
  ...
  //validation condition to allow user MI:
  validation {
    condition     = var.managed_identities.system_assigned || length(var.managed_identities.user_assigned_resource_ids) > 0
    error_message = "At least one of system_assigned or user_assigned_resource_ids must be specified."
  }
...
variable "always_ready" {
  description = "Always ready instances for Flex Consumption Function App."
  type = map(object({
    name           = optional(string)
...
variable "resource_type_code" {
  type        = string
  description = "(Required) Azure resource type abbreviation. Example: `app`."
  default     = "app"
}
