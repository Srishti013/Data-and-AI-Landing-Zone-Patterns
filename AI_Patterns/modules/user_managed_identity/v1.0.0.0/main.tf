...
module "module_umi" {
  ...
  additional_tags = merge(
    var.additional_tags != null ? var.additional_tags : {},
    {
      ...
      Status         = var.status
      ProductName    = "user_managed_identity"
...
resource "azurerm_user_assigned_identity" "this" {
  location            = module.module_umi.location
  name                = module.module_umi.name
  resource_group_name = var.resource_group_name
  tags                = module.module_umi.tags
}
...
resource "azurerm_management_lock" "this" {
  ...
  name       = coalesce(var.lock.name, "lock-${module.module_umi.name}")
